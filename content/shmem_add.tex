...
\begin{Csynopsis}
int shmem_int_add(int *dest, int value, int pe);
int shmem_long_add(long *dest, long value, int pe);
int shmem_longlong_add(long long *dest, long long value, int pe);
\end{Csynopsis}

\begin{Fsynopsis}
INTEGER pe, errcode
INTEGER*4  value_i4
errcode = SHMEM_INT4_ADD(dest, value_i4, pe)
INTEGER*8 value_i8
errcode = SHMEM_INT8_ADD(dest, value_i8, pe)
\end{Fsynopsis}
