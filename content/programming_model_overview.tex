...
\begin{enumerate}
  \item \OPR{Notification}: The \ac{PE} reports that some operation has encountered 
      an error. Errors are reported through error codes returned from \openshmem 
      operations, and through registered callbacks.
  \item \OPR{Registration}: The \ac{PE} registers the error callback to be triggered
      when \openshmem reports an error code.
  \item \OPR{Interruption}: The \ac{PE} interrupts the normal flow of execution at all 
