...
\subsection{Error Management Routines}
\openshmem provides a set of \acp{API} for managing errors. The
\acp{API} allow one to capture errors resulting from underlying failures to
achieve the defined semantic of \openshmem operations.

\subsubsection{\textbf{SHMEM\_ERRHANDLER\_SET}}\label{subsec:shmem_errhandler_set}
\input{content/shmem_errhandler_set}

%\subsubsection{\textbf{SHMEM\_ERRHANDLER\_GET}}\label{subsec:shmem_errhandler_get}
%\input{content/shmem_errhandler_get}

\subsubsection{\textbf{SHMEM\_ERRHANDLER\_CALL}}\label{subsec:shmem_errhandler_call}
\input{content/shmem_errhandler_call}
